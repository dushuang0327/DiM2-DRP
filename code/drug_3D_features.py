...
def mol_to_3d_from_smiles(smiles):
    ...
    def _get_atom_poses(mol):
        """返回经优化的 new_mol 以及对应的三维坐标 (N,3)"""
        atom_poses = []
        try:
            new_mol = Chem.AddHs(mol)
            cids = AllChem.EmbedMultipleConfs(new_mol, numConfs=10)
            res = AllChem.MMFFOptimizeMoleculeConfs(new_mol)
            index = int(np.argmin([x[1] for x in res]))
        ...
        for i, atom in enumerate(new_mol.GetAtoms()):
            if atom.GetAtomicNum() == 0:
                # 保持返回签名一致：返回 (new_mol, positions)
                zeros = np.zeros((new_mol.GetNumAtoms(), 3), dtype=np.float32)
                return new_mol, zeros
            pos = conf.GetAtomPosition(i)
            atom_poses.append([pos.x, pos.y, pos.z])

        return new_mol, np.array(atom_poses, dtype=np.float32)
    ...
    edge_attr_atom = _get_edge_features(new_mol, bond_length)
    ...
    drug_atom.pos = torch.from_numpy(atom_poses)
